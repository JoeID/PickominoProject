...
import copy
import pickle
from pickomino_project3 import *
from utils import *
...
def gradient_descent(dices, dominos):
    alpha_player = 1
    beta_player = 1
    alphas = [alpha_player]
    betas = [beta_player]

    epsilon = 0.1 # discretization parameter
    gamma = 0.01 # gradient descent step length
    N = 10 # number of games simulation to estimate average reward

    def Reward(alpha, beta):
        # Estimates average reward against (1, 1) opponent
        average_scores = [0,0]
        for _ in range(N):
            scores = simulate_game(dices, dominos, alpha, beta, 1, 1)
            average_scores[0] += scores[0]
            average_scores[1] += scores[1]
        
        average_scores[0] /= N
        average_scores[1] /= N
        #print(f"We get in average {average_scores[0]}, opponent gets in average {average_scores[1]}")
        return average_scores[0] - average_scores[1]

    for _ in range(15):
        print(f"alpha = {alpha_player}, beta = {beta_player}")

        R_a_b = Reward(alpha_player, beta_player)
        R_a_eps_b = Reward(alpha_player + epsilon, beta_player)
        R_a_b_eps = Reward(alpha_player, beta_player + epsilon)

        reward_gradient = [(R_a_eps_b - R_a_b) / epsilon, (R_a_b_eps - R_a_b) / epsilon]
        alpha_player -= gamma * reward_gradient[0]
        alpha_player = max(alpha_player, 0)
        alpha_player = min(alpha_player, 2)
        beta_player -= gamma * reward_gradient[1]
        beta_player = max(beta_player, 0)
        beta_player = min(beta_player, 2)

        alphas.append(alpha_player)
        betas.append(beta_player)
...
def simulate_game(dices, dominos, alpha_player, beta_player, alpha_opponent, beta_opponent):
    opponent_dominos = []
    player_dominos = []
    board_dominos = copy.copy(dominos)
...
gradient_descent(4, [(11, 1), (12, 1), (13, 2), (14, 2), (15, 3), (16, 3), (17, 4), (18, 4)])